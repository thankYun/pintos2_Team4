...
/* Obtains a single free page and returns its kernel virtual
   ...
   available, returns a null pointer, unless PAL_ASSERT is set in
   FLAGS, in which case the kernel panics. 
   하나의 빈 페이지를 얻고 해당 페이지의 커널 가상 주소를 반환합니다.
 PAL_USER가 설정된 경우 페이지는 사용자 풀에서 가져오고, 그렇지 않으면 커널 풀에서 가져옵니다.
 FLAGS에 PAL_ZERO가 설정된 경우 페이지는 0으로 채워집니다. 페이지를 사용할 수 없는 경우,
 PAL_ASSERT가 FLAGS에 설정된 경우를 제외하고는 널 포인터를 반환합니다. 이 경우 커널이 패닉 상태가 됩니다.*/
void *
